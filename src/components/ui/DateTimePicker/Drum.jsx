...
export default function Drum({ items, value, isDisabled, onChange, label }) {
  ...
  const handleScroll = () => {
    ...
    timer.current = setTimeout(() => {
      ...
      const final = fwd < items.length ? fwd : bwd >= 0 ? bwd : idx

      ref.current.scrollTo({ top: final * ITEM_H, behavior: 'smooth' })
      if (items[final] !== undefined) onChange(items[final])
    }, 120)
  }
  ...
  return (
    <div className="flex flex-col items-center gap-2 flex-1">
      ...
      <div style={{ position: 'relative', height: ITEM_H * 5, width: '100%', overflow: 'hidden' }}>
        {/* Highlight band */}
        <div style={{ position: 'absolute', top: ITEM_H * 2, left: 8, right: 8, height: ITEM_H,
        ...
        <div ref={ref} onScroll={handleScroll}
          ...
          {items.map(item => {
            ...
            return (
              <div key={item} style={{ height: ITEM_H, scrollSnapAlign: 'center',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: 'var(--font-display)', fontSize: '2rem', letterSpacing: '0.04em',
